...
export class AuthService {
  ...
  async login(dto: { login: string; password: string }): Promise<{
    ...
    if (!user) {
      return this.loginPlatformUser(dto, lockoutKey, normalizedLogin);
    }
    ...
    if (user.tenant && !user.tenant.actif) {
      throw new UnauthorizedException('TENANT_INACTIF');
    }

    if (user.role === 'PARENT') {
      throw new ForbiddenException('PARENT_LOGIN_DISABLED');
    }
